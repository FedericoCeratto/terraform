...
func TestAccAWSALB_accesslogs(t *testing.T) {
	var conf elbv2.LoadBalancer
	bucketName := fmt.Sprintf("testaccawsalbaccesslogsbucket-%s", acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum))
	albName := fmt.Sprintf("testaccawsalb-accesslogs-%s", acctest.RandStringFromCharSet(6, acctest.CharSetAlphaNum))
...
data "aws_elb_account_id" "current" {}
...
data "aws_iam_policy_document" "logs_bucket" {
  statement {
    ...
    principals = {
      type        = "AWS"
      identifiers = ["arn:aws:iam::${data.aws_elb_account_id.current.id}:root"]
    }
  }
}
